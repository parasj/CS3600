...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextx, nexty = int(x + dx), int(y + dy)
            if not self.walls[nextx][nexty]:
                if (nextx, nexty) in self.corners and (nextx, nexty) not in c:
                    c = c + ((nextx, nexty),)
...
def cornersHeuristic(state, problem):
    ...
    # Heuristic - find all corners to visit, find straight line manhattan distance to get to each
    (pos, c) = state
    pendingCorners = set(corners) - set(c)
    
    h = 0
    while len(pendingCorners) is not 0:
        (cost, nextpos) = getNearest(pos, pendingCorners)
        h += cost
        pendingCorners.remove(nextpos)
        pos = nextpos
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        searchFunction = lambda prob: search.breadthFirstSearch(prob) # fixed cost shortest path
